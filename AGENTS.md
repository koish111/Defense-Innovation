...
- **Guard presentation (2026-09-14):** tap right-click plays the customer sword/shield parry once; release lets that short animation finish. Continued holding returns to the original classic sword/shield guard pose without a client toggle. Shield counter resources play on server-confirmed bash; fortified resources play on Power Guard and use separate static PG loops. The left sword parry temporarily uses the unchanged right-arm resource; TODO: replace it with the customer-authored left variant. Other left/right resources follow physical handedness, and dual Power Guard composes both participating arms. Guard snapshots confirm both item components and PG state to the defender and tracking players. Stun, depletion, replacement, respawn and level exit invalidate affected presentation. Animation timing does not extend server defense windows.
...
**First-person guard animation chain (2026-09-14):** customer one-shots and Power Guard use PAL `THIRD_PERSON_MODEL` with both arms and held items enabled, including MoreRotation bends. While that layer is active the original manual render handlers yield; continuous normal guard uses `LegacyGuardPoseRenderer`, `SwordGuardArmPose` and `ShieldGuardPose`. `GuardArmTransforms.renderGuardPose` applies the vanilla BLOCK transform delta and renders through public `ItemInHandRenderer#renderItem` inside push/pop. `BashWindupPayload` identifies the defender, carries the server windup duration, and reaches that player and tracking observers once per confirmed attempt.
...
- **Guard visual lifecycle (2026-09-14):** unchanged guard heartbeats do not replay one-shots. Normal release finishes the short parry visual; bash and PG stop on guard exit. Depletion, stun, invalid equipment, respawn, disconnect and level change stop the affected layer. `RenderHandEvent` shares its matrix between hands, so manual rendering stays inside push/pop. `GuardSparks` captures actual item transforms through an added `ItemInHandLayer` for vanilla/PAL/MoreRotation and through the original first-person guard render path. Pending anchors expire after the next tick. The supplied texture is loaded into the particle atlas. ParticleStorm 1.1.4 requires `minecraft:emitter_local_space.position=true` to add the emitter world position to shape coordinates; omitting it spawns these sparks near the world origin. During solo visual acceptance, each accepted raise previews sparks; the successful-block trigger is added only at finalization.
